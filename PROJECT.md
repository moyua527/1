...
| 角色 | 说明 | 权限范围 |
|------|------|----------|
| **admin** | 管理员 | 全部功能，含用户管理、销售漏斗、数据报表、邀请码管理、商机管理 |
| **business** | 业务人员 | 客户管理（仅自己负责/创建的）、商机管理、项目/任务、站内消息 |
...
- **数据隔离**：business 角色仅能看到自己负责（assigned_to）或创建的客户
- 客户详情页：
  - 基本信息编辑（含对接人分配）
  - 联系人管理（CRUD）
  - 合同管理（CRUD）
...
| 角色 | 项目 | 客户 | 商机 | 任务 | 合同/跟进 | 站内消息 | 仪表盘 |
|------|------|------|------|------|----------|----------|--------|
| admin | 全部 | 全部 | 全部 | 全部 | 全部 | ✅ | 全部 + 漏斗 |
| business | 全部 | 负责/创建的 | 负责/创建的 | 全部 | 负责客户的 | ✅ | 全部 |
| tech | 全部 | — | — | 全部 | — | ✅ | 基础 |
