...
> 注：代码中 `permissions.tsx` 仍保留了旧角色名（sales_manager、business、tech、marketing、support、viewer、client）作为兼容映射，但当前实际分配的平台角色只有 admin 和 member。具体业务能力通过**企业权限**（企业内自定义角色）和**项目权限**（项目内企业角色绑定）两层叠加控制。
...
- **注册角色区分**：系统邀请码→member（需审批）/ 邀请链接→预设角色（直接激活）
