...
- **用户设置三Tab**：「账号与安全」（密码/设备/绑定手机邮箱）、「偏好设置」（外观主题/声音提示及音量/语言时区）、「通知设置」（按模块分组：需求管理/项目管理/客户管理/系统安全，每组独立开关+计数器）
...
- **任务详情弹窗**：编辑标题/描述/状态/优先级/指派人/截止日期、附件管理、删除任务，并显示创建人/创建时间（精确到秒）
...
- **资料库分类操作**：编辑模式根据选中分类（图片/文档/网址/表格/音视频/其他）显示对应操作按钮和上传过滤
- **网址书签**：资料库支持添加网址书签（标题 + URL + 引导备注），以 `text/x-url` 存储在文件表中
- **资料组**：资料库支持「资料组」模式，每个资料组可包含多种类型内容（网址、文字、文件、图片）；创建时可设置可见性（全部可见/指定成员）；只有创建人可以添加/删除内容、修改可见性、删除资料组
- 文件上传（multipart/form-data，限制 100MB / 20个文件）
...
- **用户备注名**：每个用户可为其他用户设置私有备注名（类似微信/QQ），备注名在项目成员、消息、私信、企业成员、群聊等全局优先显示；成员信息弹窗支持设置/修改/清除备注名；`user_nicknames` 表 + `useNicknameStore` 全局缓存
- Enter 发送
...
- **邀请链接管理**：生成平台成员注册链接（固定为 member）+ 有效期，支持复制/查看状态/删除
- **注册邀请码管理卡片**：查看（眼睛图标显示/隐藏）+ 复制 + 修改 + 关闭邀请码
- **注册角色区分**：系统邀请码→member（需审批）/ 邀请链接→member（直接激活）
- **角色修改即时生效**：管理员在用户管理中修改平台角色后，后续请求立即按数据库最新角色判定，无需等待用户重新登录
...
- **文章管理**：创建/编辑/删除文章，支持标题、正文、分类、标签；文章状态分「草稿」和「已发布」
- **分类管理**：企业级分类体系，支持新增/删除分类；左侧分类树快速筛选
- **全文搜索**：标题、内容、标签模糊搜索，支持状态和分类组合筛选
- **阅读统计**：每篇文章记录浏览次数
