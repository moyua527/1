...
- **站内消息（Direct Message）**：用户间一对一聊天、对话列表 + 未读计数、WebSocket 实时推送新消息（发送时服务端 emit new_dm 事件）、搜索用户发起新对话
...
- **Service Worker**：静态资源 cache-first 策略，API 请求 network-first 策略
...
| 共享 UI 组件 | 11 个 |
| 前端路由 | 14 条 |
| 版本迭代 | 89 个（v1.0.0 ~ v1.0.88，详见 CHANGELOG.md） |
