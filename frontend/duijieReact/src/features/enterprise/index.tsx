...
import { Building2, Building, Phone, Mail, Users, MapPin, Clock, Briefcase, FileText, Edit3, UserPlus, X, Trash2, Plus, ChevronRight, ChevronDown, Hash, Calendar, GitBranch, FolderTree, MoreHorizontal, Search, LogIn, Globe, Shield, Crown, UserCog, Eye } from 'lucide-react'
...
export default function Enterprise() {
  ...
  if (!data) return (
    <div>
      ...
      {/* 系统管理员：所有企业列表（只读） */}
      {isSysAdmin && allEnterprises.length > 0 && (
        <div style={{ marginTop: 24 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {allEnterprises.map((item: any) => {
              const ae = item.enterprise
              const ams = item.members || []
              const ads = item.departments || []
              const isExp = expandedEntId === ae.id
              const gdn = (id: number) => ads.find((d: any) => d.id === id)?.name || ''
              return (
                <div key={ae.id} style={{ ...section, padding: 0, overflow: 'hidden' }}>
                  <div onClick={() => setExpandedEntId(isExp ? null : ae.id)}
                    style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 20px', cursor: 'pointer', background: isExp ? '#f8fafc' : '#fff' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                        <span style={{ fontSize: 16, fontWeight: 700, color: '#0f172a' }}>{ae.name}</span>
                        {ae.company_type && <span style={{ fontSize: 10, padding: '1px 6px', borderRadius: 4, background: '#e0e7ff', color: '#3730a3' }}>{ae.company_type}</span>}
                        {ae.industry && <span style={{ fontSize: 10, padding: '1px 6px', borderRadius: 4, background: '#f0fdf4', color: '#15803d' }}>{ae.industry}</span>}
                      </div>
                      {ae.company && <div style={{ fontSize: 12, color: '#64748b', marginTop: 1 }}>{ae.company}</div>}
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                      <span style={{ fontSize: 12, color: '#94a3b8' }}><Users size={12} style={{ verticalAlign: -2, marginRight: 3 }} />{ams.length}人</span>
                      <span style={{ fontSize: 12, color: '#94a3b8' }}><Building size={12} style={{ verticalAlign: -2, marginRight: 3 }} />{ads.length}部门</span>
                      {ae.creator_name && <span style={{ fontSize: 11, color: '#9333ea', background: '#f3e8ff', padding: '1px 6px', borderRadius: 4 }}>创建者: {ae.creator_name}</span>}
                    </div>
                  </div>
                  {isExp && (
                    <div style={{ padding: '0 20px 16px', borderTop: '1px solid #e2e8f0' }}>
                      <div style={{ paddingTop: 12, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '2px 24px', marginBottom: 12 }}>
                        {ae.credit_code && <div style={infoRow}><Hash size={14} color="#64748b" /> <span style={{ color: '#94a3b8', fontSize: 12 }}>信用代码</span> <span style={{ fontFamily: 'monospace', fontSize: 12 }}>{ae.credit_code}</span></div>}
                        {ae.legal_person && <div style={infoRow}><Users size={14} color="#64748b" /> <span style={{ color: '#94a3b8', fontSize: 12 }}>法人</span> {ae.legal_person}</div>}
                        {ae.phone && <div style={infoRow}><Phone size={14} color="#64748b" /> {ae.phone}</div>}
                        {ae.email && <div style={infoRow}><Mail size={14} color="#64748b" /> {ae.email}</div>}
                        {ae.address && <div style={infoRow}><MapPin size={14} color="#64748b" /> {ae.address}</div>}
                      </div>
                      {ams.length > 0 && (
                        <>
                          <div style={{ fontSize: 13, fontWeight: 600, color: '#334155', marginBottom: 8 }}>成员 ({ams.length})</div>
                          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 8 }}>
                            {ams.map((m: any) => {
                              const mr = m.role || 'member'
                              const rcc = roleConfig[mr] || roleConfig.member
                              return (
                                <div key={m.id} style={{ border: '1px solid #f1f5f9', borderRadius: 8, padding: 10, display: 'flex', alignItems: 'center', gap: 8, borderLeft: mr === 'creator' ? '3px solid #9333ea' : mr === 'admin' ? '3px solid #2563eb' : '1px solid #f1f5f9' }}>
                                  <Avatar name={m.name} size={32} />
                                  <div style={{ flex: 1, minWidth: 0 }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                                      <span style={{ fontSize: 13, fontWeight: 600, color: '#0f172a' }}>{m.name}</span>
                                      <span style={{ fontSize: 9, padding: '0px 5px', borderRadius: 3, background: rcc.bg, color: rcc.color, fontWeight: 600 }}>{rcc.label}</span>
                                    </div>
                                    <div style={{ fontSize: 11, color: '#94a3b8' }}>
                                      {m.position && <span>{m.position} </span>}
                                      {m.department_id && <span>· {gdn(m.department_id)}</span>}
                                    </div>
                                  </div>
                                </div>
                              )
