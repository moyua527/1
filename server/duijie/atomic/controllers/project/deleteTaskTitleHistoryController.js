...
module.exports = async (req, res) => {
  try {
    ...
    const [deleted] = await db.query(
      'DELETE FROM duijie_task_title_history WHERE id = ? AND project_id = ? AND user_id = ? LIMIT 1',
      [req.params.historyId, req.params.id, req.userId]
    );
    if (!deleted.affectedRows) return res.status(404).json({ success: false, message: '历史记录不存在' });
