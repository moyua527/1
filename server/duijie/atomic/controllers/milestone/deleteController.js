...
module.exports = async (req, res) => {
  try {
    const { id } = req.params;
    const [[ms]] = await db.query('SELECT project_id FROM duijie_milestones WHERE id = ? AND is_deleted = 0', [id]);
    ...
    await db.query('UPDATE duijie_milestones SET is_deleted = 1 WHERE id = ?', [id]);
