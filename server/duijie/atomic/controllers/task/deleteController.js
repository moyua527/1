...
module.exports = async (req, res) => {
  try {
    const [rows] = await db.query('SELECT project_id, status, title FROM duijie_tasks WHERE id = ? AND is_deleted = 0', [req.params.id]);
    ...
    if (rows[0].project_id) {
      logActivity(rows[0].project_id, req.userId, 'task_deleted', { entityType: 'task', entityId: Number(req.params.id), title: rows[0].title });
    }
