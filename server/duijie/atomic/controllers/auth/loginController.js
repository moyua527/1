...
module.exports = async (req, res) => {
  try {
    ...
    if (result.disabled) {
      recordLogin(result.user?.id || 0, { loginType: 'password', ip: req.ip, userAgent: req.headers['user-agent'], status: 'failed', failReason: '账号已被禁用' });
      return res.status(403).json({ success: false, message: '账号已被禁用，请联系管理员' });
    }
    recordLogin(result.user.id, { loginType: 'password', ip: req.ip, userAgent: req.headers['user-agent'], status: 'success' });
