...
module.exports = async (req, res) => {
  try {
    const { username, password, nickname, email, phone, invite_code } = req.body;
    ...
    const [result] = await db.query(
      'INSERT INTO voice_users (username, password, nickname, email, phone, role) VALUES (?, ?, ?, ?, ?, ?)',
      [username.trim(), password, displayName, email || null, phone || null, 'member']
    );
