...
module.exports = async (req, res) => {
  try {
    ...
    const [users] = await db.query('SELECT phone FROM voice_users WHERE id = ? AND is_active = 1', [userId]);
    if (users.length === 0) return res.status(400).json({ success: false, message: '用户不存在' });
    const phone = users[0].phone;
    if (!phone) return res.status(400).json({ success: false, message: '请先绑定手机号后再修改密码' });
    ...
    logger.info(`用户 ${userId} 通过手机验证修改了密码`);
