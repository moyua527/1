...
| 版本 | 日期 | 说明 |
|------|------|------|
| v1.1.71 | 2026-04-03 | **项目角色 60 项细粒度权限**：将原 16 个粗粒度权限拆分为 60 个操作级权限字段，按 17 个分组管理（项目信息/客户关联/成员管理/角色管理/任务创建/删除/状态流转/编辑/附件/审核要点/预设标题/里程碑/报表/应用）；迁移文件 030 自动将旧数据展开到新字段；企业角色保持 16 字段不变，代码层 ER_TO_PR_MAP 映射展开；projectPermGuard 支持数组 OR 逻辑；前端角色管理 UI 按分组显示权限开关，支持全选/分组全选；默认角色：负责人（60/60）、编辑者（27/60 任务相关）、查看者（0/60） |
