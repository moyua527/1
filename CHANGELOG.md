...
| v1.1.47 | 2026-03-28 | 登录会话热修：修复网页登录后刷新回到登录页的问题；后端 `/api/auth/me` 改为仅查询稳定用户字段，避免老环境因 `voice_users` 扩展字段缺失导致 500；前端刷新初始化统一通过 `authApi.me()` 恢复会话，并统一共享 token/fetch 入口；新增 `015_add_last_login_at.sql` 与 `016_sync_voice_users_profile_columns.sql` 迁移补齐登录与用户资料相关字段。 |
