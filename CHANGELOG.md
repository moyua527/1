...
| v1.1.48 | 2026-03-29 | 刷新链路继续热修：前端 `useUserStore.init()` 将鉴权恢复与 `/api/my-enterprise` 解耦，只在明确 401/403/404 时清空登录态，修复多次刷新后被误踢回登录页；项目详情页补充加载失败态、8 秒超时与重试按钮，修复 `/projects/:id?tab=overview` 在刷新场景下偶发卡死“加载中”的问题；同步统一前端、Android 与后端版本号入口到 `1.1.48`。 |
